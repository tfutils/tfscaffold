...
understand the bug, write or update a test, fix the code, verify manually,
...
Write or identify a test that demonstrates the bug. Verify it fails before
the fix and passes after. Note: tfscaffold does not have a formal test
suite — verification is manual via dry-run invocations.
...
bin/terraform.sh -a plan -p testproject -c example -e dev -r eu-west-2
```

Read the full output carefully and check for regressions.
