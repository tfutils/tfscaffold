...
2. Trace every code path through `bin/terraform.sh`
3. Check quoting, operator precedence, cross-platform compatibility
4. Verify error handling — what happens when AWS calls fail, files are
   missing, regions are invalid?
5. Check tfvars file parsing edge cases (empty, duplicates, missing)
6. Review S3 backend configuration for security issues
7. Check bootstrap destroy safety and state management paths
8. Audit hook scripts (pre.sh/post.sh) for injection risks
