...
3. Classify each commit: BREAKING CHANGE, FEATURE, BUG FIX, CHORE
...
3. Update `script_ver` in `bin/terraform.sh` to match the new version
4. Commit changes
...
2. `git tag -a vX.Y.Z -m "tfscaffold vX.Y.Z"` on the merge commit
