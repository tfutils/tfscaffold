locals {
  ...
  # UI Customisation
  identity_provider_name = coalesce(try(var.ui_customisation["name"], null), local.unique_id)

  # Cognito User Pool
  user_pool_client               = local.saml_idp ? aws_cognito_user_pool_client.saml[0] : aws_cognito_user_pool_client.cognito[0]
  user_pool_client_callback_urls = length(var.callback_urls) > 0 ? var.callback_urls : ["https://${var.app_fqdn}/oauth2/idpresponse"]
